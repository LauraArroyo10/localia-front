...
function BusinessCard({ business, onViewMore, size }: BusinessCardProps) {
  ...
  className={`flex items-center justify-center bg-violet-50 rounded-full absolute cursor-pointer z-10
    ${isSmall ? "w-9 h-9 top-4 right-4 text-2xl" : "w-14 h-14 top-7 right-7 text-4xl"}`}
>
          {isFavorite ? (
            <FaHeart size={isSmall ? 16 : 24} className="text-terracota-500 " />
          ) : (
            <FaRegHeart size={isSmall ? 16 : 24} className="text-violet-900" />
          )}
        </div>
      </div>
      ...
      <div className="px-7 pt-1 pb-7">
        ...
        <div className="py-2">
          <p className="text-terracota-400 text-lg">{business.location}</p>
