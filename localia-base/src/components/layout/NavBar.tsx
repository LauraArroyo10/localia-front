...
function NavBar({ onLoginClick, onRegisterClick }: NavBarProps) {
  ...
  <div className=" max-w-[1150px] mx-auto flex items-center justify-between h-13">
    ...
    <div className="flex items-center gap-1">
      ...
      <button onClick={onLoginClick} className="text-sm text-violet-700 px-4 py-1.5 hover:bg-violet-50 rounded-3xl">
        Sign In
      </button>
      <button onClick={onRegisterClick} className="text-sm font-medium text-violet-700 border border-violet-700 px-4 py-1.5 rounded-3xl hover:bg-violet-50">
        Sign Up
      </button>
    </div>
